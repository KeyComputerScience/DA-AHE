...
def expand_a(seed: bytes, params: LatticeParams) -> np.ndarray:
    byte_count = params.k * params.k * params.d * 4
    raw = hashlib.shake_256(b"DA-AHE/A/" + seed).digest(byte_count)
    return np.frombuffer(raw, dtype="<u4").astype(np.int64).reshape(
        params.k, params.k, params.d
    )
